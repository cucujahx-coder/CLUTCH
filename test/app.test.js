...
async function common(file){
 ...
 assert(/\.grp\{[^}]*border-radius:24px;overflow:hidden;\s*background:var\(--surf-row\)/.test(css)&&/\.row\{[^}]*background:transparent;border-radius:0/.test(css)&&/\.row \+ \.row::after\{[^}]*left:18px;right:42px/.test(css),'задачи собраны в одну плашку: стекло у группы, строки прозрачные, между ними волосок от текста');
