...
async function common(file){
 ...
 assert(/--rowh:56px/.test(read('app.css'))&&/\.row\{[^}]*height:var\(--rowh\);padding:6px 6px 6px 16px[^}]*border-radius:28px/.test(read('app.css'))&&/\.pri\{[^}]*height:var\(--rowh\)[^}]*border-radius:28px/.test(read('app.css')),'строка 56: кружок 44 как у круглых кнопок + 6×2, радиус 28, капсула приоритета той же высоты');
 assert(!/\.row \.ck\{/.test(read('app.css'))&&/\.ck\{[^}]*width:44px;height:44px/.test(read('app.css'))&&/\.pri button\{[^}]*width:44px;height:44px/.test(read('app.css'))&&/\.row \.t1\{font-size:15px;line-height:18px\}/.test(read('app.css'))&&/\.row \.t2\{font-size:12px;line-height:14px/.test(read('app.css'))&&/#list\{[^}]*gap:4px/.test(read('app.css')),'кружок общий 44, заголовок 15/18, подпись 12/14, зазор 4');
