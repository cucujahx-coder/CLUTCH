...
async function common(file){
 ...
 assert(/\.row\{[^}]*height:56px;padding:6px 20px 6px 6px/.test(read('app.css'))&&/\.pri\{[^}]*height:56px/.test(read('app.css')),'строка компактная, 56 = 44 + 6×2, капсула приоритета той же высоты');
 ...
 assert(/\.row\{[^}]*flex-direction:row-reverse[^}]*padding:6px 20px 6px 6px/.test(css)&&/\.step\{[^}]*flex-direction:row-reverse/.test(css),'кружки слева: строки и шаги перевёрнуты, отступ текста справа');
