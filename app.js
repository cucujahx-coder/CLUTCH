...
function paint(keep){
 ...
 if(S.showDone){
  ...
  /* Порядок считается от якоря списка (кнопка #sort): срочное — ближе к якорю, новое —
     ближе к якорю внутри своего приоритета. От кнопки (S.up = 0): срочное внизу, у пальца,
     новое внизу своей ступени. Под логотипом (S.up = 1) — ровно зеркально: срочное наверху,
     новое наверху своей ступени. Задачи и проекты одним рядом (id общий). Один на все виды. */
  items.sort(S.up ? (a,b)=>(b.pri - a.pri) || (b.id - a.id) : (a,b)=>(a.pri - b.pri) || (a.id - b.id));
...
/* Порядок списка: снизу вверх от кнопки (по умолчанию) или сверху вниз под логотипом.
   Меняется и якорь, и направление приоритетов (см. сортировку в paint): срочное всегда у
   якоря. Класс .top у экрана переворачивает контейнер (column вместо column-reverse). */
function drawSort(){
