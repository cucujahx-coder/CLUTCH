...
/* Логотип в большой кнопке — крест из четырёх квадратов с пустой серединой (v123, образец
   владельца). Рисуется теми же «пикселями», что и всё остальное, только квадратными: сетка
   3×3, заняты верх, низ, лево и право, углы и центр пусты. Класс .sw оставлен — на нём висят
   сжатие под пальцем, переход в микрофон в режиме голоса и перекраска в красный на вкладке FOCUS. */
const LOGO_SQ =
 '<span class="sw"><svg viewBox="0 0 3 3" aria-hidden="true"><g fill="currentColor">'+
 [[1,0],[0,1],[2,1],[1,2]].map(([x,y])=>'<rect x="'+x+'" y="'+y+'" width="1" height="1"/>').join('')+
 '</g></svg></span>';
